...
class ResNet_MT(ReaNet):
    ...
    def __init__(self, in_channels, out_channels=512):
        ...
        layers = [3, 4, 6, 3]
        block = BasicBlock
        output_channel_block = [int(out_channels / 16), int(out_channels / 8), int(out_channels / 4),
                                int(out_channels / 2), out_channels]

        self.inplanes = output_channel_block[0]
        self.conv0 = BasicConv(in_channels, output_channel_block[0], kernel_size=3, stride=1, padding=1, bias=False,
                               use_bn=True, use_relu=True)
        ...
        self.conv4 = BasicConv(output_channel_block[4], out_channels, kernel_size=2, stride=2, bias=False,
