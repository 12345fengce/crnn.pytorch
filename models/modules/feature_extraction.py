...
class ResNet(nn.Module):
    def __init__(self, in_channels, **kwargs):
        ...
        self.features = nn.Sequential(
            ...
            BasicBlock(in_channels=channels[0], out_channels=channels[2], expand_size=expand_size[0], kernel_size=3, stride=1),
            BasicBlock(in_channels=channels[2], out_channels=channels[3], expand_size=expand_size[1], kernel_size=3, stride=1),
            nn.Dropout(0.2),

            BasicBlock(in_channels=channels[3], out_channels=channels[4], expand_size=expand_size[2], kernel_size=3, stride=2),
            BasicBlock(in_channels=channels[4], out_channels=channels[5], expand_size=expand_size[3], kernel_size=3, stride=1),
            nn.Dropout(0.2),

            nn.Conv2d(in_channels=channels[5], out_channels=channels[6], kernel_size=2, stride=(2, 1), padding=(0, 1), bias=False),

            BasicBlock(in_channels=channels[6], out_channels=channels[7], expand_size=expand_size[4], kernel_size=3, stride=1),
            nn.BatchNorm2d(512),
            nn.ReLU(),
